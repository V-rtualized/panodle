...
export const decompressGameData = (compressedGuesses) => {
  return compressedGuesses.map(guess => ({
    ...
    comparison: {
      ...
      Difficulty: guess.c.d === 'c' ? 'correct' : guess.c.d === 'h' ? 'higher' : 'lower',
      Features: guess.c.f === 'c' ? 'correct' : guess.c.f === 'p' ? 'partial' : 'incorrect',
      Length: guess.c.ln === 'c' ? 'correct' : guess.c.ln === 'h' ? 'higher' : 'lower',
      StartingElevation: guess.c.s === 'c' ? 'correct' : guess.c.s === 'h' ? 'higher' : 'lower',
      EndingElevation: guess.c.e === 'c' ? 'correct' : guess.c.e === 'h' ? 'higher' : 'lower'
    }
  }));
};

export const saveGameState = (setCookie, targetRun, newGuesses, won = false) => {
