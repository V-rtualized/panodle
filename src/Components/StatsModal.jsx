...
const StatsModal = ({ isOpen, onClose, attempts }) => {
  ...
  const getLast7DaysData = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const attemptsValue = date.toISOString().split('T')[0] === today.toISOString().split('T')[0]
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-md w-full relative">
        ...
        <div className="mb-4 text-center">
          <p className="text-lg">You found the run in <span className="font-bold">{attempts}</span> attempts!</p>
        </div>

        <div className="w-full h-48">
        <BarChart width={350} height={180} data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
          <XAxis 
            dataKey="label" 
            tick={{ fontSize: 12 }}
            interval={0}
          ...
          />
          <Tooltip 
            formatter={(value) => value === 0 ? 'No attempt' : `${value} attempts`}
          />
          <Bar 
            dataKey="attempts" 
            fill="#3B82F6"
